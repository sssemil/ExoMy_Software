...
def get_shovel_motor_pins():
    shovel_motor_pins = {}
    with open(config_filename, 'r') as file:
        param_dict = yaml.load(file)

    for param_key, param_value in param_dict.items():
        if('pin_shovel_' in str(param_key)):
            shovel_motor_pins[param_key] = param_value
    return shovel_motor_pins


def get_shovel_pwm_neutral_values():
    shovel_pwm_neutral_values = {}
    with open(config_filename, 'r') as file:
        param_dict = yaml.load(file)

    for param_key, param_value in param_dict.items():
        if('shovel_pwm_neutral_' in str(param_key)):
            shovel_pwm_neutral_values[param_key] = param_value
    return shovel_pwm_neutral_values


def get_position_name(name):
    position_name = ''
    if('_sl' in name):
        position_name = 'Shovel Left'
    elif('_sr' in name):
        position_name = 'Shovel Right'

    return position_name


def update_config_file(shovel_pwm_neutral_dict):
    output = ''
    with open(config_filename, 'rt') as file:
        for line in file:
            for key, value in shovel_pwm_neutral_dict.items():
                if(key in line):
...
\________|\__/  \__| \______/ \__|     \__| \____$$ |
    ...
    print(
        '''
This script helps you to set the neutral pwm values for the shovel motors.
You will iterate over all shovel motors and set them to a neutral position.
...
------------------------------------------------------------------------------
    ...
    # Get all shovel pins
    shovel_motor_pins = get_shovel_motor_pins()
    pwm_neutral_dict = get_shovel_pwm_neutral_values()
    # Iterating over all motors and fine tune the zero value
    for pin_name, pin_value in shovel_motor_pins.items():
        pwm_neutral_name = pin_name.replace(
            'pin_shovel_', 'shovel_pwm_neutral_')
        pwm_neutral_value = pwm_neutral_dict[pwm_neutral_name]

        print('Set ' + get_position_name(pin_name) + ' shovel motor: \n')
        while(1):
            # Set motor
